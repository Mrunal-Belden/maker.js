...
namespace MakerJs {
  ...
  export function isPathLine(item: any): boolean {
    return (
      isPath(item) &&
      (<IPath>item).type == pathType.Line &&
      isPoint((<IPathLine>item).end)
    );
  }
  ...
  export function isPathCircle(item: any): boolean {
    return (
      isPath(item) &&
      (<IPath>item).type == pathType.Circle &&
      isNumber((<IPathCircle>item).radius)
    );
  }
  ...
  export function isPathArc(item: any): boolean {
    return (
      isPath(item) &&
      (<IPath>item).type == pathType.Arc &&
      isNumber((<IPathArc>item).radius) &&
      isNumber((<IPathArc>item).startAngle) &&
      isNumber((<IPathArc>item).endAngle)
    );
  }
  ...
  export function isPathArcInBezierCurve(item: any): boolean {
    return (
      isPathArc(item) &&
      isObject((<IPathArcInBezierCurve>item).bezierData) &&
      isNumber((<IPathArcInBezierCurve>item).bezierData.startT) &&
      isNumber((<IPathArcInBezierCurve>item).bezierData.endT)
    );
  }
  ...
  export interface IPathIntersectionOptions
    extends IPathIntersectionBaseOptions {
    /**
     * Optional boolean to only return deep intersections, i.e. not on an end point or tangent.
     */
    path1Offset?: IPoint;
